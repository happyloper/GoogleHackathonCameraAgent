...
저는 **메이커**로서 다양한 제작 과정을 촬영하는 일이 많습니다. 3D 프린터, 납땜, 조립 등 작업 중에는 **멀티 카메라 전환**이나 **특정 부품 클로즈업** 같은 촬영이 필수적인데, 혼자서 작업하면서 동시에 카메라 구도까지 잡는 것은 매우 힘든 일입니다.

그래서 **Google Gemini AI**의 비전 인식과 음성 제어 기능을 활용하여, **손가락으로 가리키고 음성으로 명령하면 AI가 자동으로 카메라를 조정해주는 촬영 보조 시스템**을 만들어 보았습니다.

> 해커톤 특성상 실제 로봇팔 하드웨어를 가져올 수 없어, **소프트웨어만으로 타겟 지정 → 좌표 추출 → 로봇팔 이동 메커니즘**을 구현하여 시연합니다. AI가 감지한 바운딩 박스 좌표를 로봇팔 컨트롤러에 전달하면 물리적 카메라가 자동으로 해당 물체를 추적하는 방식으로 확장됩니다.
